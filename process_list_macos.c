...
// read VmRSS from proc_pidinfo
static long unsigned int get_memory_usage(const char *pid_str) {
    pid_t pid = atoi(pid_str);
    struct proc_taskinfo proc;

    int st = proc_pidinfo(pid, PROC_PIDTASKINFO, 0, &proc, sizeof(proc));
    if (st <= 0) {
        return 0;
    }

    return proc.pti_resident_size / 1024;
}
...
static void get_system_cpu_times(unsigned long long *total, unsigned long long *idle_out) {
    ...
    if (kr == KERN_SUCCESS) {
        *idle_out = cpu_load.cpu_ticks[CPU_STATE_IDLE];
        *total = cpu_load.cpu_ticks[CPU_STATE_USER] + cpu_load.cpu_ticks[CPU_STATE_NICE] + cpu_load.cpu_ticks[CPU_STATE_SYSTEM] + cpu_load.cpu_ticks[CPU_STATE_IDLE];
    }
}
...
void get_system_info(SystemInfo *info, ProcessList *list, ProcessList *prev_list) {
    ...
    size_t len = sizeof(total_mem_bytes);
    sysctlbyname("hw.memsize", &total_mem_bytes, &len, NULL, 0);
    ...
    if (host_statistics64(mach_host_self(), HOST_VM_INFO64, (host_info_t)&vm_stats, &count) != KERN_SUCCESS) {
        return;
    }

    struct xsw_usage swap;
    size_t swap_len = sizeof(swap);
    sysctlbyname("vm.swapusage", &swap, &swap_len, NULL, 0);

    #define TO_KB(bytes) (bytes / 1024)
    #define PG_TO_KB(pg) ((uint64_t)pg * page_size / 1024)

    info->mem_total = TO_KB(total_mem_bytes);
    info->mem_free  = PG_TO_KB(vm_stats.free_count);

    info->mem_cached = PG_TO_KB(vm_stats.inactive_count);
    info->mem_available = info->mem_free + info->mem_cached;
    info->swap_total = TO_KB(swap.xsu_total);
    info->swap_free  = TO_KB(swap.xsu_avail);
    ...
    if (sysctl(mib, 2, &boottime, &size, NULL, 0) != -1 && boottime.tv_sec != 0) {
        ...
        info->uptime = (double)(now - boottime.tv_sec);
    }
    ...
    for (int i = 0; i < (int)core_count && i < 32; i++) {
        unsigned long long user   = cpu_load[i].cpu_ticks[CPU_STATE_USER];
        unsigned long long system = cpu_load[i].cpu_ticks[CPU_STATE_SYSTEM];
        ...
        if (list->core_old_totals[i] > 0) {
            unsigned long long diff_total = total - list->core_old_totals[i];
            unsigned long long diff_idle  = idle_total - list->core_old_idles[i];

            if (diff_total > 0) {
                info->core_percents[i] = (float)(diff_total - diff_idle) / diff_total * 100.0f;
            } else {
                info->core_percents[i] = 0.0f;
            }
        }

        list->core_old_totals[i] = total;
        list->core_old_idles[i]  = idle_total;
    }

    info->core_count = (int) core_count;
    vm_deallocate(mach_task_self(), (vm_address_t)cpu_load, processor_msg_count * sizeof(int));
